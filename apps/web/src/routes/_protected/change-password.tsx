...
export const Route = createFileRoute("/_protected/change-password")({ component: ChangePassword });
	...
	return (
		<div className="mx-auto flex min-h-screen max-w-sm flex-col gap-4 bg-background p-6">
			<header className="flex items-center gap-2">
				<button onClick={() => navigate({ to: "/" })} className="text-text-secondary">
					<ChevronRight size={24} />
				</button>
				<h1 className="text-[20px] font-bold text-text">{t("change.title")}</h1>
			</header>
			<form onSubmit={save} className="flex flex-col gap-3">
				<TextInput icon={<Lock size={20} />} type="password" placeholder={t("change.current")} value={current} onChange={(e) => setCurrent(e.target.value)} required />
				<TextInput icon={<Lock size={20} />} type="password" placeholder={t("change.new")} value={next} onChange={(e) => setNext(e.target.value)} required />
