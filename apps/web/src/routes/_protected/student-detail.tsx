...
export const Route = createFileRoute("/_protected/student-detail")({
	...
	loader: async ({ deps }) => getStudentDetail({ data: { studentId: deps.studentId } }),
...
function StudentDetail() {
	...
	if (!student) return <div className="p-6 text-center text-text-secondary">—</div>;
		...
		<div className="mx-auto flex min-h-screen max-w-md flex-col gap-4 bg-background p-4">
			...
			<Card className="flex items-center justify-between">
				<div className="flex flex-col">
					<span className="text-[15px] font-bold text-text">{student.name}</span>
					<span className="text-[12px] text-text-secondary">{student.email}</span>
				</div>
			...
			<Card className="flex flex-col gap-2">
				<div className="flex items-center justify-between">
					<span className="text-[13px] font-bold text-text">{t("detail.plan")}</span>
					<div className="flex gap-2">
						...
						{plan ? (
							<button onClick={remove} className="rounded-md bg-error/10 px-2 py-1 text-[12px] font-semibold text-error">
								{t("detail.remove")}
							</button>
						) : null}
					</div>
				...
				{plan ? (
					...
					<span className="text-[13px] text-text-light">{t("detail.noPlan")}</span>
				)}
			</Card>

			<ReviewProgress reviews={reviews} sessions={sessions} points={student.points} streak={student.streak} />
		</div>
	);
}
