...
export function ReviewProgress({
	...
	const onTime = reviews.filter((r) => r.status === "completed" && r.pointsEarned === 10).length;
	...
	return (
		<div className="flex flex-col gap-4">
			...
			{tab === "reviews" ? (
				<>
					<div className="flex gap-2">
						<StatCard value={points} label={t("progress.points")} tone="primary" onClick={() => setMetric("points")} />
						...
						<StatCard value={streak} label={t("progress.streak")} tone="secondary" />
					</div>
					<Card>
						<span className="mb-1 block text-[12px] font-semibold text-text-secondary">{t(`progress.${metric}`)}</span>
						{series.length > 0 ? (
						...
						{reviews.length === 0 ? (
							...
							[...reviews].reverse().map((r) => (
								<div key={r.id} className="flex items-center gap-2 rounded-md border border-border bg-surface px-3 py-2 text-[12px]">
									<span className={`h-2 w-2 rounded-full ${r.status === "completed" ? "bg-success" : r.status === "missed" ? "bg-error" : "bg-text-light"}`} />
									<span className="flex-1 font-semibold text-text">{r.surahName}: {r.verseFrom}–{r.verseTo}</span>
				...
				<div className="flex flex-col gap-2">
					<div className="flex gap-2">
						<StatCard value={sessions.length} label={t("progress.sessionCount")} tone="secondary" />
					</div>
					{sessions.length === 0 ? (
						<p className="py-8 text-center text-[13px] text-text-secondary">{t("progress.noData")}</p>
					) : (
						[...sessions].reverse().map((s) => (
							<div key={s.id} className="flex items-center gap-2 rounded-md border border-border bg-surface px-3 py-2 text-[12px]">
								...
								<span className="flex-1 font-semibold text-text">{s.memorizedSurah}: {s.memorizedVerseFrom}–{s.memorizedVerseTo}</span>
								<span className="text-text-light">{s.sessionDate}</span>
								{s.evaluation ? (
									<span className="rounded bg-accent-light px-1.5 py-0.5 text-[10px] font-bold text-[#9A7A2E]">{t(`eval.${s.evaluation}`)}</span>
								) : null}
							</div>
						))
					)}
